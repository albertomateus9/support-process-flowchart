...
DATA_SAMPLE = ROOT / "data" / "sample" / "support_process.json"
DATA_PROCESSED = ROOT / "data" / "processed"
CHARTS = ROOT / "charts"
REPORTS = ROOT / "reports"
PROJECT = {
  "code": "R-03",
  "slug": "support-process-flowchart",
  "title": "Support Process Flowchart",
  "kind": "support_flowchart",
  "description": "Support-process modeling lab that renders a triage, diagnosis, resolution, and closure flowchart."
}
...
def run_faq_chatbot() -> dict:
    ...
    for prompt in prompts:
        answer = "No synthetic answer found."
        for rule in rules:
...
def run_iot_simulator() -> dict:
    df = read_csv()
    df["alert"] = np.where((df["temperature_c"] > 29) | (df["humidity_percent"] < 60), "review", "normal")
    df.plot(x="timestamp", y=["temperature_c", "humidity_percent"], marker="o", figsize=(8, 4), title="Synthetic classroom IoT")
    plt.xticks(rotation=25, ha="right")
    return {"rows": len(df), "alerts": int((df["alert"] == "review").sum()), "outputs": [save_df(df, "iot_alerts.csv"), save_chart("iot_timeseries.png")]}
...
def run_ai_heatmap() -> dict:
    ...
    plt.colorbar(label="Synthetic index")
...
def run_inventory_dashboard() -> dict:
    ...
    status.pivot(index="lab", columns="status", values="total").fillna(0).plot(kind="bar", stacked=True, figsize=(7, 4), title="Lab inventory status")
    report = "<h1>Inventory Summary</h1>" + status.to_html(index=False)
...
def run_gantt() -> dict:
    ...
    axis.set_xlabel("Days from project start")
...
def run_requirements_report() -> dict:
    ...
    lines = ["# Requirements Specification", "", "Synthetic interview responses grouped for classroom practice.", ""]
    for _, row in df.iterrows():
...
def run_privacy_analyzer() -> dict:
    ...
    counts.plot(kind="bar", x="term", y="count", figsize=(7, 4), legend=False, title="LGPD vocabulary sample")
...
def run_connection_quality() -> dict:
    df = read_csv()
    df["sla_status"] = np.where((df["download_mbps"] >= 150) & (df["latency_ms"] <= 40), "ok", "review")
    df.plot(x="timestamp", y=["download_mbps", "latency_ms"], marker="o", figsize=(8, 4), title="Connection quality sample")
    plt.xticks(rotation=25, ha="right")
    return {"rows": len(df), "review_points": int((df["sla_status"] == "review").sum()), "outputs": [save_df(df, "connection_quality.csv"), save_chart("connection_quality.png")]}
